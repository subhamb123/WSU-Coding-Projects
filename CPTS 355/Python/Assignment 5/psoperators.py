...
class PSOperators:
    ...
    def define(self,name, value):
        ...
        if(isinstance(self.dictstack[len(self.dictstack) - 1][1],DictionaryValue)):
            self.dictstack[len(self.dictstack) - 1][1].value[name] = value
        else:
        ...
        if self.scope == "static":
            for i in range(self.index):
                if isinstance(self.dictstack[i][1],dict):
                    value = self.dictstack[i][1].get("/" + name, None)
                    if value != None:
                        return value
                else:
                    value = self.dictstack[i][1].value.get("/" + name, None)
                    if value != None:
                        return value
            ...
            for d in reversed(self.dictstack):
                if isinstance(d[1],dict):
                    value = d[1].get("/" + name, None)
                    if value != None:
                        return value
                else:
                    value = d[1].value.get("/" + name, None)
                    if value != None:
                        return value
    ...
    def stack(self):
        ...
        print(CEND)
        print(RED+"===**dictstack**===")
        for line in reversed(range(len(self.dictstack))):
            print("----" + str(line) + "----0----")
            for i in range(len(self.dictstack[line][1])):
                print(str(list(self.dictstack[line][1].keys())[i]) + " " + str(list(self.dictstack[line][1].values())[i]))
