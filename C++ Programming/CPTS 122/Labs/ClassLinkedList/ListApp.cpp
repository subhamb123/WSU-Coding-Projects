...
ofstream& operator<< (ofstream& output, List& rhs) {
	double high = 0, low = 999999999999, mean = 0, i = 0;
	ListNode* temp = rhs.getHeadPtr();

	while (temp != nullptr) {
		if (temp->getData() > high)
			high = temp->getData();
		else if(temp->getData() < low)
			low = temp->getData();
		mean += temp->getData();
	...
	output << "High: " << high << endl << "Low: " << low << endl << "Average: " << mean << endl;
